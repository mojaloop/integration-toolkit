...
![ITK architecture and its relationship with an MDK-deployed Hub](diagrams/itk-architecture.svg)
...
**Certificate operations are automated end to end.** Enrolment, renewal, and zero-downtime rotation are the agent's job, not a runbook's. The class of outage caused by expired or mis-installed certificates disappears, at the cost of one requirement: the agent must be running for renewals to happen.
...
| Keep alive | Because |
|---|---|
| The MCM Agent container | It drives certificate renewal; if it is down, an expiring certificate will not renew |
